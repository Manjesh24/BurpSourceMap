#
#  BurpSourceMap - Parse JavaScript source maps
#
...
class BurpExtender(IBurpExtender, IScannerCheck, ITab):
    def registerExtenderCallbacks(self, callbacks):
        ...
        callbacks.setExtensionName("BurpSourceMap")

        callbacks.issueAlert("BurpSourceMap Passive Scanner enabled")
        ...
        print ("BurpSourceMap loaded.")
        print ("Copyright (c) 2023 Manjesh S")
        self.outputTxtArea.setText("BurpSourceMap Running :" + "\n")

    def initUI(self):
        self.tab = swing.JPanel()

        # UI for Output
        self.outputLabel = swing.JLabel("BurpSourceMap Log:")
    ...
    def doPassiveScan(self, checkSourceMap):
        ...
        try:
            ...
            if ".js" in str(urlReq):
                ...
                headers = [h.replace('.js HTTP', '.js.map HTTP') for h in headers]
                ...
                self.outputTxtArea.append("\n" + "[+] Version " + str(version) + " source map with "+str(len(sources))+" sources found:")
                issues = ArrayList()
...
class ScanIssue(IScanIssue,ITab):
    ...
    def getIssueDetail(self):
        return str("Burp Scanner has analysed the following Javascript Source map: <b>"
                      "%s.map</b><br><br><i>Check the sitemap to view Unpacked Javascript code.</i></b><br><br>" % (self.reqres.getUrl().toString()))
